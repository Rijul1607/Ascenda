...
import { Book, Download, FileText, Star } from "lucide-react"
...
const ResourceGrid = ({ resources }) => {
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {resources.map((resource) => <ResourceCard key={resource.id} resource={resource} icon={resource.id.startsWith("book") ? <Book className="h-6 w-6" /> : <FileText className="h-6 w-6" />} />)}
    </div>
  );
};
...
const ResourceCard = ({ resource, icon }) => {
  ...
  return (
    <Card className="overflow-hidden transition-all hover:border-primary transition-colors duration-300">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-xl">{icon} {resource.title}</CardTitle>
      ...
      <CardContent>
        ...
        <div className="mt-2 text-sm text-muted-foreground">Category: {resource.category}{resource.language && `, Language: ${resource.language}`}</div>
      </CardContent>
